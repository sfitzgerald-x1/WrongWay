...
/// Third generation (2026-09-05): re-baselined once more when `origin/main`
/// (#29-#35: subtree visits, the true ply and repetition window carried in the
/// record) was merged with the `v3` qtransform branch. #35 changes the record
/// bytes every digest below hashes, so the pins moved for a documented reason
/// unrelated to D3; the `eps = 0` sweep still agrees across every alpha, which
/// is the claim this file makes. Re-pinned from the merged build's own output.
const PRE_D3_ARGMAX_DIGEST: u64 = 0x806b_2c1b_3fce_3128;
const PRE_D3_TEMPERATURE_DIGEST: u64 = 0x70fc_9969_d3a8_28b8;
...
const FLOORED_TEMPERATURE_DIGEST: u64 = 0x816e_ddcd_1597_eefa;
