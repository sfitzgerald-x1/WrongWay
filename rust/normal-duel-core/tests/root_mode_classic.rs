...
/// Third generation (2026-09-05): re-baselined when `origin/main` (#29-#35, which
/// carries the true ply and repetition window in every record) was merged with
/// the `v3` qtransform branch. The record bytes moved for that documented reason;
/// the claim -- the default Gumbel root is untouched by the classic arm -- is
/// re-asserted against the merged build's own output. Same values as
/// `tests/dirichlet_root.rs` pins, by construction.
const GUMBEL_ARGMAX_DIGEST: u64 = 0x806b_2c1b_3fce_3128;
const GUMBEL_TEMPERATURE_DIGEST: u64 = 0x70fc_9969_d3a8_28b8;
