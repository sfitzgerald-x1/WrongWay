...
fn an_extracted_root_carries_the_window_a_fresh_search_would_have() {
    ...
    assert!(
        checked >= 10,
        "only {checked} extractions were actually checked"
    );
}
...
fn extraction_survives_being_applied_twice_in_a_row() {
    ...
    let sub = search
        .into_subtree(&config, &[first_result.action_code])
        .expect("own move");

    let mut resumed =
        PuctTreeSearch::resume(&config, sub, params(), Lcg32::new(32)).expect("resumes");
...
fn a_resumed_search_inherits_a_tree_and_still_completes() {
    ...
    assert!(
        inherited > 1,
        "extraction produced a bare root, nothing was inherited"
    );
    assert!(
        inherited < before,
        "the subtree cannot be larger than the tree it came from"
    );

    let mut resumed =
        PuctTreeSearch::resume(&config, sub, params(), Lcg32::new(5)).expect("resumes");
    ...
    assert_eq!(
        out.simulations_used, 256,
        "a resumed search must spend its own budget"
    );
    assert!(
        out.visit_counts.iter().any(|(_, v)| *v > 0),
        "no candidate was visited"
    );
...
fn a_path_that_was_never_expanded_yields_nothing() {
    ...
    assert!(
        search.into_subtree(&config, &[9_999]).is_none(),
        "an unknown code must miss"
    );
}
...
fn a_restart_keeps_inherited_value_but_not_inherited_exploration() {
    ...
    for (visits, since) in search.root_edge_visits() {
        assert_eq!(
            visits, since,
            "a fresh search must keep the two counters identical"
        );
    }
