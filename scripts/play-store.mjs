 ...
 *
 *   tailscale  the tailnet's own identity, stamped onto the request by `tailscale
 *            serve` after WireGuard has authenticated the device. Requires no sign-in,
 *            and is the default. The server accepts it only from loopback -- which is
 *            sufficient in the tailnet deployment, where the proxy is the only route
...
export function createStore({ dir, googleClientId = '', httpsOrigin = false }) {
   ...
   *
   * WireGuard authenticates the device before HTTP exists, and `tailscale serve`
   * stamps the login onto the request it proxies, so there is no token and nothing to
   * sign in to. The caller checks the peer is loopback before trusting the header.
   *
   * That check is weaker than it looks, and the caller documents why: loopback proves
   * "local", not "came from the proxy", so on a machine where the port is directly
   * reachable a caller can assert any login. What is exposed is leaderboard integrity
   * only. WW_PLAY_PROXY_SECRET closes it.
   */
  function identify({ login, name }) {
    // Bound and sanitise the login before it becomes a user id. It arrives from a
    // header, so an unbounded value would let a caller mint unlimited distinct users
    // into the append-only file, and control bytes would corrupt the JSONL besides.
    const clean = String(login).toLowerCase().replace(/[^a-z0-9._@+-]/g, '').slice(0, 64);
    ...
    // Strip control characters: the display name also comes from a header, and a
    // newline in it would split one JSONL record into two unparseable halves.
    const nice = String(name || clean).replace(/[\x00-\x1f\x7f]/g, '').trim()
      .slice(0, 40) || clean;
  ...
  /**
   * A stable, opaque handle for a user.
   *
   * The client needs to recognise its OWN row in the standings, and matching on display
   * name is wrong: names are not unique, so a collision showed someone else's record as
   * yours. The raw id cannot be used for that because it carries the tailnet login
   * (`ts:someone@example.com`), which nothing on the page should publish. A keyed hash
   * gives the client something stable to compare without revealing who anyone is.
   */
  const handle = (id) => createHmac('sha256', secret).update(`uid:${id}`).digest('hex').slice(0, 16);

  const publicUser = (u) => u && ({ id: handle(u.id), name: u.name,
                                    verified: u.verified, provider: u.provider });
  ...
  function leaderboard(recentLimit = 10) {
    ...
    for (const g of games) {
      const e = per.get(g.userId)
        || { userId: handle(g.userId), name: g.name, verified: g.verified,
             w: 0, l: 0, d: 0, games: 0, last: null };
    ...
    return {
      // `recent` rows go out through handle() too. The players array was fixed and this
      // was not, so /api/leaderboard -- which has no auth -- published raw tailnet
      // logins (ts:someone@example.com) for every recent game. handle()'s own docstring
      // says nothing on the page should publish those.
      recent: games.slice(-recentLimit).reverse()
        .map((g) => ({ ...g, userId: handle(g.userId) })),
      players, totals,
